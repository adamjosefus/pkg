...
import { parseTokenCliArgument, TokenType } from "./model/tokens/mod.ts"
...
function tokensFlag(v?: string | null): TokenType[] {
    return parseTokenCliArgument(v ?? null)
}
...
export function getArguments(root: string) {
    const args = new Arguments(
        ...
        {
            name: 'update, reinstall, u',
            description: `Shortcut for "--uninstall" and "--install".`,
            convertor: booleanFlag,
        },
        {
    ...
    args.setDescription(`Packager is a tool for managing your packages.`)
    ...
    if (args.get<boolean>('version')) {
        throw new InfoInterruption([
            bold(`Packager ${formatVersion(currentVersion)}`),
    ...
    return {
        install: args.get<ReturnType<typeof booleanFlag>>('install') || args.get<ReturnType<typeof booleanFlag>>('update'),
        uninstall: args.get<ReturnType<typeof booleanFlag>>('uninstall') || args.get<ReturnType<typeof booleanFlag>>('update'),
